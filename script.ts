    ...
    for (const tabel of tabels) {
        for (const accrual of accruals) {
            if (accrual.data.tabel
                ...
                // Проект, задача, задание, период, специализация
                const [accrual_task, accrual_project, accrual_projectTask, accrual_period, specialization] = await Promise.all([
                    accrual.data.zadanie?.fetch(),
                    accrual.data.project?.fetch(),
                    accrual.data.zadanie?.fetch().then(task => task.data.project_task?.fetch()),
                    accrual.data.period?.fetch(),
